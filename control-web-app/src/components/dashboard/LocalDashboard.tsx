...
function rangoFechas(periodo: Periodo): { desde: string; hasta: string; anterior_desde: string; anterior_hasta: string } {
  ...
  if (periodo === "hoy") {
    ...
    return { desde: hasta, hasta, anterior_desde: isoDate(ayer), anterior_hasta: isoDate(ayer) };
  }
  if (periodo === "semana") {
...
const PERIODO_LABELS: Record<Periodo, string> = { hoy: "Hoy", semana: "7 días", mes: "Este mes" };
...
export default function LocalDashboard() {
  ...
  const cargarStats = useCallback(async (manual = false) => {
    ...
    try {
      ...
      const listaAnt: Venta[] = ventasAntRes.data.results ?? ventasAntRes.data;
      const totalPeriodo = lista.reduce((a, v) => a + Number(v.total), 0);
      const totalAnterior = listaAnt.reduce((a, v) => a + Number(v.total), 0);
      ...
      const gastos_periodo = listaGastos.reduce((a, g) => a + Number(g.monto), 0);
      const ticket_promedio = lista.length ? totalPeriodo / lista.length : 0;
  ...
  useEffect(() => {
    ...
    cargarStats();
    if (esAdmin()) {
      api.get("/suscripciones/mi-suscripcion/").then((r) => setSuscripcion(r.data)).catch(() => {});
    }
  ...
  const trend = stats && stats.total_anterior > 0
    ? ((stats.total_periodo - stats.total_anterior) / stats.total_anterior) * 100
    : undefined;
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm xl:col-span-2">
            ...
            <div className="px-5 pb-5">
              {loading ? (
                ...
                <div className="h-44 flex flex-col items-center justify-center text-muted-foreground/30">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm">
            ...
            <div className="px-5 pb-5">
              {loading ? (
                  ...
                  <div className="flex justify-center my-2">
                    <PieChart width={110} height={110}>
                      <Pie data={stats.metodos_pago} dataKey="total" cx={55} cy={55}
                        outerRadius={50} innerRadius={30} strokeWidth={2} stroke="hsl(var(--card))">
                ...
                <div className="h-36 flex items-center justify-center text-sm text-muted-foreground font-medium">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            {loading ? (
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                          <Clock size={10} /> {formatDateTime(v.fecha)}
                        </p>
                      </div>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center h-36 text-muted-foreground/30">
